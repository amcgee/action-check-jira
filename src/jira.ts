...
import { JiraAPIPaginatedResponse, JiraIssue, JiraProject } from "./jiraTypes";
...
export async function getProjectKeys() {
  const projects = await fetchJira<JiraAPIPaginatedResponse<JiraProject>>(
    "/project/search?status=live"
  );
...
export async function getJiraIssue(key: string) {
  const issue = await fetchJira<JiraIssue>(
    `/issue/${key}?fields=labels,summary`
  );
  return issue;
}
